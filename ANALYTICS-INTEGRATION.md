...
- GTM未公開ワークスペースのプレビューで、Local Googleタグ `G-H68SYXZ86L` が1回発火し、Production Googleタグ `G-TWNC4EVTYD` が発火していないことをTag Assistantで確認した。
...
- ローカル判定の除外条件は特定ポートへ固定せず、`localhost` と `127.0.0.1` の任意ポートをLocalとして扱うよう、未公開ワークスペースで更新した。

現時点ではGTMコンテナとバージョンは未公開。標準イベントと環境分離の実受信は確認済みだが、カスタムイベント3種は未完了のため、計測全体を最終PASSとは扱わない。
